...
const MainPage = () => {
    ...
    return garden.length ? (
        ...
        >
          <p className="mainList__inputs__name">irrigation</p>
          <input type="text" placeholder={plant.irrigation} onChange={(e) => setIrr(e.target.va) />
          <button onClick={() => setInterval(()=>dispatch({type: 'STARTTIME_PLANT', id: plant.id}),1000)}>Start timer</button>
          <p className="mainList__inputs__name">fertilization</p>
          <input type="text"/>
          <p className="mainList__inputs__name">weeding</p>
          <input type="text"/>
