...
const NewPlantForm = () => {
    ...
    return (
        <form className="addPage" onSubmit={handleSubmit}>
            ...
            <input className="addPage_input-type-file" type="file" accept="file/*" onChange={(e) => setImage(URL.createObjectURL(e.target.files[0]))} />
            <input className="addPage_input-type-submit" type="submit" value="Add plant"/>
            {garden.length >= 1 ? <Link to="/mainPage" className="navigation--addPage">Move to Garden Panel</Link> : ''}
        </form>
     );
}
