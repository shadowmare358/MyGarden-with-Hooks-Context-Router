...
const MainPage = () => {
    ...
    const handleTime = (plant) => {
      let counter = 0;
      let fun = () => {
        if(counter >= 99){
          clearInterval(id);
          alert('Irrigation needed! ')
          dispatch({type: 'IRRIGATION_RESET', id: plant.id});
        }else{
          counter++;
          dispatch({type: 'STARTTIME_PLANT', id: plant.id});
        }
      }
      let id = setInterval(fun, 10)
    ...
    return garden.length ? (
        ...
        >
          ...
          <button onClick={() => handleTime(plant)}>Start timer</button>
          <button onClick={() => dispatch(plant)}>Start timer</button>
          <p className="mainList__inputs__name">fertilization</p>
          <input type="text"/>
          <p className="mainList__inputs__name">weeding</p>
          <input type="text"/>
