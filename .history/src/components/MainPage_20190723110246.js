...
const MainPage = () => {
    ...
    return garden.length ? (
        ...
        >
          ...
          <button onClick={() => dispatch({type: 'IRRIGATION_RESET', id: plant.id})}>RESET</button>
          <p className="mainList__inputs__name">fertilization</p>
          <input type="text"/>
          <button onClick={() => handleTime(plant,'FERTILIZATION')}>Start timer</button>
          <button onClick={() => dispatch({type: 'FERTILIZATION_RESET', id: plant.id})}>Start timer</button>
          <p className="mainList__inputs__name">weeding</p>
          <input type="text"/>
          <button onClick={() => handleTime(plant,'WEEDING')}>Start timer</button>
          <button onClick={() => dispatch({type: 'WEEDING_RESET', id: plant.id})}>Start timer</button>
